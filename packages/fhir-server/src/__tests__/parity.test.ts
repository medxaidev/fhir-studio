...
// =============================================================================
// 6ï¸âƒ£ Error Handling Parity (5 tests)
// =============================================================================
...
// =============================================================================
// 7ï¸âƒ£ Concurrency Stress Parity (3 tests)
// =============================================================================
...
// =============================================================================
// 8ï¸âƒ£ Database Layer Parity (4 tests)
// =============================================================================
...
// =============================================================================
// 9ï¸âƒ£ Extreme/Edge Parity (6 tests)
// =============================================================================
...
describe("9. Extreme/Edge Parity", () => {
  ...
  it("EX-P-04: Unicode (CJK, emoji, RTL) preserved on both", async () => {
    const body = {
      resourceType: "Patient",
      name: [{ family: "å¼ ä¸‰", given: ["æŽå››"] }, { family: "Ù…Ø±Ø­Ø¨Ø§", given: ["Ø¹Ø§Ù„Ù…"] }, { family: "ðŸ¥ðŸ©º", given: ["ðŸ’‰"] }],
    };
    const mx = await medxaiRequest("POST", "/Patient", body);
    const mp = await medplumRequest("POST", "/Patient", body);
    expect(mx.status).toBe(201);
    ...
    const mxR = await medxaiRequest("GET", `/Patient/${mx.body.id}`);
    const mpR = await medplumRequest("GET", `/Patient/${mp.body.id}`);
    expect(mxR.body.name[0].family).toBe("å¼ ä¸‰");
    expect(mpR.body.name[0].family).toBe("å¼ ä¸‰");
    expect(mxR.body.name[1].family).toBe("Ù…Ø±Ø­Ø¨Ø§");
    expect(mpR.body.name[1].family).toBe("Ù…Ø±Ø­Ø¨Ø§");
    expect(mxR.body.name[2].family).toBe("ðŸ¥ðŸ©º");
    expect(mpR.body.name[2].family).toBe("ðŸ¥ðŸ©º");
    trackBoth("Patient", mx.body.id, mp.body.id);
  });
