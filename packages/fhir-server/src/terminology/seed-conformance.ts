 ...
 * - CodeSystem  â€?from valuesets.json, v3-codesystems.json, v2-tables.json
 * - ValueSet    â€?from valuesets.json, v3-codesystems.json, v2-tables.json
 * - StructureDefinition â€?from profiles-types.json, profiles-resources.json, profiles-others.json
 *
...
export async function seedConformanceResources(
    ...
    if (platformDir) {
      for (const fileName of ["valuesets-medxai.json", "codesystems-medxai.json"]) {
        try {
          ...
          // Custom files may not exist yet â€?that's OK
        }
      }
    }
  }
  ...
  for (const [, resource] of deduped) {
    ...
    // DB id column is UUID â€?convert non-UUID spec IDs to deterministic UUIDs
    ...
    try {
      if (skipExisting) {
        ...
        try {
          ...
          // Does not exist â€?proceed to create
        }
      }
